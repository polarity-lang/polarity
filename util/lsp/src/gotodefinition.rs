...
trait ToJumpContent {
    fn to_jump_content(&self) -> Option<Location>;
}

impl ToJumpContent for InfoContent {
    fn to_jump_content(&self) -> Option<Location> {
        match self {
            InfoContent::VariableInfo(i) => i.to_jump_content(),
            InfoContent::TypeCtorInfo(i) => i.to_jump_content(),
            InfoContent::CallInfo(i) => i.to_jump_content(),
            InfoContent::DotCallInfo(i) => i.to_jump_content(),
            InfoContent::TypeUnivInfo(i) => i.to_jump_content(),
            InfoContent::HoleInfo(i) => i.to_jump_content(),
            InfoContent::AnnoInfo(i) => i.to_jump_content(),
        }
    }
}
