...
fn check_command() {
    let mut cmd = Command::cargo_bin(BINARY).unwrap();
    let assert = cmd.current_dir("..").args(vec!["check", "examples/encoding_scott.pol"]).assert();
...
fn check_command_2() {
    let mut cmd = Command::cargo_bin(BINARY).unwrap();
    let assert = cmd.current_dir("..").args(vec!["check", "examples/encoding_church.pol"]).assert();
