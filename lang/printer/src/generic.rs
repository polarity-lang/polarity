...
impl<'a, P: Phase> Print<'a> for ParamInst<P> {
    fn print(&'a self, _cfg: &PrintCfg, alloc: &'a Alloc<'a>) -> Builder<'a> {
        let ParamInst { info: _, span: _, name, typ: _ } = self;
        alloc.text(name)
    }
}
...
impl<'a, P: Phase> Print<'a> for TypApp<P> {
    fn print(&'a self, cfg: &PrintCfg, alloc: &'a Alloc<'a>) -> Builder<'a> {
        let TypApp { info: _, span: _, name, args } = self;
...
impl<'a, P: Phase> Print<'a> for Exp<P> {
    fn print_prec(&'a self, cfg: &PrintCfg, alloc: &'a Alloc<'a>, prec: Precedence) -> Builder<'a> {
        match self {
            Exp::Var { info: _, span: _, name, ctx: _, idx } => {
                if cfg.de_bruijn {
            ...
            Exp::TypCtor { info: _, span: _, name, args } => {
            ...
            Exp::Ctor { info: _, span: _, name, args } => {
                let psubst = if args.is_empty() { alloc.nil() } else { args.print(cfg, alloc) };
            ...
            mut dtor @ Exp::Dtor { .. } => {
                ...
                while let Exp::Dtor { info: _, span: _, exp, name, args } = &dtor {
                    let psubst = if args.is_empty() { alloc.nil() } else { args.print(cfg, alloc) };
            ...
            Exp::Anno { info: _, span: _, exp, typ } => {
                exp.print(cfg, alloc).parens().append(COLON).append(typ.print(cfg, alloc))
            }
            Exp::Type { info: _, span: _ } => alloc.keyword(TYPE),
            Exp::Match { info: _, span: _, ctx: _, name, on_exp, motive, ret_typ: _, body } => {
                on_exp
                    .print(cfg, alloc)
                    .append(DOT)
            ...
            Exp::Comatch { info: _, span: _, ctx: _, name, is_lambda_sugar, body } => {
