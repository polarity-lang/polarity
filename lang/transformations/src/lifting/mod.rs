...
impl Ctx {
    ...
    fn lift_match(
        ...
        let cases = cases.subst_new(&self.ctx, &subst);
        let self_typ = self_typ.subst_new(&self.ctx, &subst);
        let def_ret_typ = match &motive {
            Some(m) => m.lift(self).subst_new(&self.ctx, &subst).ret_typ,
            None => shift_and_clone(
                &ret_typ.clone().unwrap().lift(self).subst_new(&self.ctx, &subst),
                (1, 0),
            ),
        };
    ...
    fn lift_comatch(
        ...
        // Substitute the new parameters for the free variables
        let cases = cases.subst_new(&self.ctx, &subst);
        let typ = typ.subst_new(&self.ctx, &subst);
