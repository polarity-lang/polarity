...
pub trait Mapper<P: Phase> {
    ...
    fn map_data(&mut self, info: P::Info, name: Ident, ignored: bool, typ: Rc<TypAbs<P>>, ctors: Vec<Ident>) -> Data<P> {
        Data { info, name, ignored, typ, ctors }
    }
    fn map_codata(&mut self, info: P::Info, name: Ident, ignored: bool, typ: Rc<TypAbs<P>>, dtors: Vec<Ident>) -> Codata<P> {
        Codata { info, name, ignored, typ, dtors }
    }
    ...
    fn map_def(&mut self, info: P::Info, name: Ident, ignored: bool, params: Telescope<P>, self_param: SelfParam<P>, ret_typ: Rc<Exp<P>>, body: Match<P>) -> Def<P> {
        Def { info, name, ignored, params, self_param, ret_typ, body }
    }
    fn map_codef(&mut self, info: P::Info, name: Ident, ignored: bool, params: Telescope<P>, typ: TypApp<P>, body: Comatch<P>) -> Codef<P> {
        Codef { info, name, ignored, params, typ, body }
    }
...
impl<P: Phase, T: Mapper<P>> Folder<P, Id<P>> for T {
    ...
    fn fold_data(&mut self, info: <Id<P> as Out>::Info, name: Ident, ignored: bool, typ: <Id<P> as Out>::TypAbs, ctors: Vec<Ident>) -> <Id<P> as Out>::Data {
        self.map_data(info, name, ignored, Rc::new(typ), ctors)
    }

    fn fold_codata(&mut self, info: <Id<P> as Out>::Info, name: Ident, ignored: bool, typ: <Id<P> as Out>::TypAbs, dtors: Vec<Ident>) -> <Id<P> as Out>::Codata {
        self.map_codata(info, name, ignored, Rc::new(typ), dtors)
    }
    ...
    fn fold_def(&mut self, info: <Id<P> as Out>::Info, name: Ident, ignored: bool, params: <Id<P> as Out>::Telescope, self_param: <Id<P> as Out>::SelfParam, ret_typ: <Id<P> as Out>::Exp, body: <Id<P> as Out>::Match) -> <Id<P> as Out>::Def {
        self.map_def(info, name, ignored, params, self_param, ret_typ, body)
    }

    fn fold_codef(&mut self, info: <Id<P> as Out>::Info, name: Ident, ignored: bool, params: <Id<P> as Out>::Telescope, typ: <Id<P> as Out>::TypApp, body: <Id<P> as Out>::Comatch) -> <Id<P> as Out>::Codef {
        self.map_codef(info, name, ignored, params, typ, body)
    }
