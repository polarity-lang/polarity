...
impl CheckInfer for LocalLet {
    fn check(&self, ctx: &mut Ctx, t: &Exp) -> TcResult<Self> {
        ...
        let elem = Binder { name: name.clone(), content: Binding { typ: typ_nf, val: Some(BoundValue::LetBinding { val: bound.clone() }) }};

        // We need to shift the binder type here because we treat it as a 1-element telescope
        let body = ctx.bind_single(shift_and_clone(&elem, (1, 0)), |ctx| {
            body.check(ctx, t)
        })?;
