use std::{fs::{self, canonicalize}, path::{Path, PathBuf}};
    ...
    fn generate_html(&self) -> String {
        ...
        if !self.files.is_empty() || !self.subfolders.is_empty() {
            ...
            for file in &self.files {
                let canonical_path = fs::canonicalize(&file.path).expect("Failed to canonicalize path");
                let target_path = get_target_path(&canonical_path);
                let trimed_path = trim_windows_path_prefix(&target_path);
                html.push_str(&format!("<li class=\"file\"><a href=\"{}\">{}</a></li>", trimed_path, file.name));
            }
